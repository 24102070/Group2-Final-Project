...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['schedule_id'])) {
    ...
    $sql_schedule = "SELECT date FROM freelancer_schedules WHERE id = ?";
    ...
    if ($days_diff < 3) {
        ...
        $check_sql = "SELECT * FROM freelancer_bookings WHERE schedule_id = ? AND status = 'accept'";
        ...
        if ($check_result->num_rows > 0) {
            ...
            $check_sql = "SELECT * FROM freelancer_bookings WHERE user_id = ? AND schedule_id = ? AND status NOT IN ('cancelled', 'accept', 'reject')";
            $stmt = $conn->prepare($check_sql);
            ...
            if ($check_result->num_rows > 0) {
                ...
                $insert_sql = "INSERT INTO freelancer_bookings (user_id, freelancer_id, schedule_id, package_id, status) VALUES (?, ?, ?, ?, 'pending')";
...
$sql = "SELECT name FROM freelancers WHERE id = ?";
